...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const panel = (isMobile = false) => (
    ...
    <div className="flex h-full flex-col p-4 sm:p-5">
      ...
      <a href="/pricing/" className={`mt-5 flex items-center rounded-2xl border border-amber-300 bg-amber-50 text-amber-900 transition hover:-translate-y-0.5 dark:border-amber-400/30 dark:bg-amber-400/10 dark:text-amber-200 ${collapsed && !isMobile ? 'justify-center p-3' : 'justify-between gap-2 px-3 py-3'}`} title={collapsed && !isMobile ? 'Creator plan · ¥9.90/month' : undefined}>
        <CreditCard className="h-4 w-4 shrink-0" />
        {(!collapsed || isMobile) && <span><span className="block font-mono text-[10px] uppercase tracking-wider opacity-70">Creator plan</span><span className="text-sm font-bold">¥9.90 / month · unlimited</span></span>}
