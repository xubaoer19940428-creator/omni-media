...
import { Check, CheckCircle2, CreditCard, Loader2, ShieldCheck, Sparkles } from 'lucide-react';
...
function ConfiguredBillingPanel() {
  ...
  // Do not render the upgrade price until the account has been resolved. A
  // signed-in member must never see the free-plan copy during the brief
  // account request (or if the account endpoint is temporarily unavailable).
  if (!account) {
    return (
      <section className="tikhub-panel rounded-2xl border border-slate-200 p-8 text-center dark:border-slate-800">
        {loading ? <Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin text-blue-600 dark:text-cyan-400" /> : <CreditCard className="mx-auto mb-2 h-5 w-5 text-slate-400" />}
        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{loading ? (lang === 'zh' ? '正在检查会员状态…' : 'Checking membership status…') : (lang === 'zh' ? '暂时无法加载会员状态' : 'Membership status is temporarily unavailable')}</p>
        {!loading && <p className="mt-2 text-xs text-slate-500">{error || (lang === 'zh' ? '请稍后刷新重试。' : 'Please refresh and try again.')}</p>}
      </section>
    );
  }

  const plan = account?.plan;
  const isMember = Boolean(plan?.unlimited);
  ...
  const expiryLabel = plan?.subscription_expires_at
    ? new Intl.DateTimeFormat(lang === 'zh' ? 'zh-CN' : 'en-US', { dateStyle: 'medium' }).format(new Date(plan.subscription_expires_at))
    : null;
    ...
    <section className="tikhub-panel overflow-hidden rounded-2xl border border-blue-200 dark:border-cyan-500/20">
      <div className="grid gap-0 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="p-6 sm:p-8">
          ...
          <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-slate-950 dark:text-white">{isMember ? (lang === 'zh' ? '无限下载已开启' : 'Unlimited downloads active') : (lang === 'zh' ? '月度无限下载' : 'Monthly unlimited downloads')}</h2>
          <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-400">{isMember
            ? (lang === 'zh' ? `你的 OmniMedia 会员已激活${expiryLabel ? `，有效期至 ${expiryLabel}` : ''}。` : `Your OmniMedia membership is active${expiryLabel ? ` through ${expiryLabel}` : ''}.`)
            : (lang === 'zh' ? `PayPal 安全结账。${displayPrice}/月自动续费，订阅有效期间可无限下载，订单与当前 Clerk 账户绑定。` : `Secure PayPal checkout. ${displayPrice}/month renews automatically for unlimited downloads while your subscription is active.`)}</p>

          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <div className={`rounded-xl border p-4 ${isMember ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-500/20 dark:bg-emerald-500/10' : 'border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/70'}`}><p className="text-[10px] font-mono uppercase tracking-wider text-slate-500">{isMember ? (lang === 'zh' ? '下载权限' : 'Download access') : (lang === 'zh' ? '剩余下载' : 'Downloads left')}</p><p className="mt-1 text-2xl font-black text-slate-950 dark:text-white">{loading ? '—' : isMember ? '∞' : account?.downloads_remaining == null ? '∞' : account.downloads_remaining}</p></div>
            ...
            <div className={`rounded-xl border p-4 ${isMember ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-500/20 dark:bg-emerald-500/10' : 'border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/70'}`}><p className="text-[10px] font-mono uppercase tracking-wider text-slate-500">{lang === 'zh' ? '账户状态' : 'Account status'}</p><p className="mt-1 text-lg font-black text-slate-950 dark:text-white">{isMember ? (lang === 'zh' ? '会员已激活' : 'Active') : (lang === 'zh' ? '免费' : 'Free')}</p></div>
          </div>
        ...
        <div className="border-t border-slate-200 bg-slate-50/80 p-6 dark:border-slate-800 dark:bg-slate-900/60 lg:border-l lg:border-t-0 sm:p-8">
          {isMember ? (
            <>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-700 dark:border-emerald-500/25 dark:bg-emerald-500/10 dark:text-emerald-300"><CheckCircle2 className="h-3.5 w-3.5" />{lang === 'zh' ? '会员已激活' : 'MEMBERSHIP ACTIVE'}</div>
              <h3 className="mt-4 text-lg font-extrabold text-slate-950 dark:text-white">{plan?.name || 'OmniMedia Pro'}</h3>
              <p className="mt-2 text-sm font-semibold text-slate-700 dark:text-slate-200">{lang === 'zh' ? '无限视频下载' : 'Unlimited video downloads'}</p>
              <p className="mt-1 text-xs text-slate-500">{lang === 'zh' ? (expiryLabel ? `当前订阅有效至 ${expiryLabel}` : '当前订阅有效') : (expiryLabel ? `Active through ${expiryLabel}` : 'Your subscription is active')}</p>
              <ul className="mt-5 space-y-2 text-xs text-slate-600 dark:text-slate-300"><li className="flex items-center gap-2"><Check className="h-3.5 w-3.5 text-emerald-500" />{lang === 'zh' ? '无限视频下载' : 'Unlimited video downloads'}</li><li className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5 text-emerald-500" />{lang === 'zh' ? 'PayPal 订阅有效' : 'PayPal subscription active'}</li></ul>
              <p className="mt-6 text-center text-[10px] leading-5 text-slate-500">{lang === 'zh' ? '无需再次购买 · 订阅期间不限次数' : 'No purchase needed · unlimited while active'}</p>
            </>
          ) : (
            <>
