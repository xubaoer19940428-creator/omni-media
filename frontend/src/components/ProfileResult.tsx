...
export const ProfileResult: React.FC<ProfileResultProps> = ({
  ...
  return (
    <section className="tikhub-panel overflow-hidden rounded-2xl border border-slate-200 shadow-xl dark:border-slate-700/80">
      <div className="flex flex-col gap-4 border-b border-slate-200 bg-slate-50/90 px-5 py-4 dark:border-slate-800 dark:bg-slate-950/95 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 sm:justify-end">
          {profile.followers ? (
            <span className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 font-mono text-[10px] text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
              {compactNumber(profile.followers)} {t.profile.followers}
            </span>
          ) : null}
          {profile.posts ? (
            <span className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 font-mono text-[10px] text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
              {compactNumber(profile.posts)} {t.profile.posts}
            </span>
          ) : null}
          {profile.url && (
            <a
